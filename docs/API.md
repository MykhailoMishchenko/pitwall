...
**Запись (имена из бандлов; точные пути/тела снять при первом реальном
трансфере перед Спа — TODO):** `saveTeam`, `transferTeam`, `updateTeamName`,
чипы через `services/user/booster`. Корни: `services/user/gameplay|booster|
cards|league|minileague|leaderboard|opponentteam`.
